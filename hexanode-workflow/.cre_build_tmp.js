...
var PROCUREMENT_ID_PREFIX = "hex-";
function generateProcurementId() {
  const t = Date.now().toString(36);
  const r = Math.random().toString(36).slice(2, 8);
  return `${PROCUREMENT_ID_PREFIX}${t}-${r}`;
}
function logState(runtime2, state, data) {
  const entry = {
    procurementId: runtime2._procurementId ?? "pending",
    state,
    timestamp: new Date().toISOString(),
    data
  };
  runtime2.log(`[${state}] ${JSON.stringify(entry)}`);
}
function detectNeed(config) {
...
function searchProviders(config, need) {
  const candidates = config.providers.filter((p) => p.products.some((prod) => prod.toLowerCase() === need.product.toLowerCase()));
  if (candidates.length === 0)
    return null;
  return candidates[0];
}
var negotiateWithLLM = (sendRequester, apiKey, baseUrl, model, need, provider) => {
...
var fetchProviderOr402 = (sendRequester, url) => {
...
var fetchWithPayment = (sendRequester, url, txHash) => {
  const response = sendRequester.sendRequest({
    url,
    method: "GET",
...
function verifyFulfillment(config, data) {
...
var onProcurementTrigger = (runtime2) => {
  const config = runtime2.config;
  const procurementId = generateProcurementId();
  runtime2._procurementId = procurementId;
  runtime2.log(`HexaNode started procurement ${procurementId}`);
  runtime2.log(`Config snapshot (safe): providerBaseUrl=${config.providerBaseUrl}, chain=${config.chainName}`);
  const need = detectNeed(config);
  if (!need) {
    logState(runtime2, "needDetected", { reason: "no_items_below_threshold" });
    return { ok: false, reason: "No inventory items below threshold" };
  }
  logState(runtime2, "needDetected", { need });
  const provider = searchProviders(config, need);
  if (!provider) {
    logState(runtime2, "search", { error: "no_matching_provider" });
    return { ok: false, reason: "No provider found" };
  }
  logState(runtime2, "search", { provider: { id: provider.id, name: provider.name } });
  let terms;
  try {
    const apiKey = runtime2.getSecret({ id: "DEEPSEEK_API_KEY" }).result().value;
  ...
  logState(runtime2, "negotiate", { terms });
  ...
  const firstFetch = httpClient.sendRequest(runtime2, (req, u) => fetchProviderOr402(req, u), consensusIdenticalAggregation())(providerUrl).result();
  if (!firstFetch.is402) {
    logState(runtime2, "payment", { error: "expected_402", status: firstFetch.status });
    return { ok: false, reason: "Provider did not return 402" };
  }
  let paymentReq;
  try {
    paymentReq = JSON.parse(firstFetch.body);
  } catch {
  ...
  if (config.paymentReceiverAddress && config.paymentReceiverAddress.length > 10) {
    try {
      const reportData = encodeAbiParameters(parseAbiParameters("address,uint256"), [payTo, amount]);
      ...
      if (!rawHash || rawHash.length === 0) {
        runtime2.log("Simulation mode: no real tx hash from writeReport, using placeholder");
      }
      runtime2.log(`USDC transfer tx: ${paymentTxHash}`);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      logState(runtime2, "payment", { error: "usdc_transfer_failed", detail: msg });
      return { ok: false, reason: `Payment failed: ${msg}` };
    }
  } else {
    runtime2.log("paymentReceiverAddress not configured - using simulated hash for demo. Deploy HexaNodePaymentReceiver for real Base Sepolia USDC transfers.");
    paymentTxHash = `0x${Array(64).fill("0").map(() => Math.floor(Math.random() * 16).toString(16)).join("")}`;
  }
  const secondFetch = httpClient.sendRequest(runtime2, (req, u, h) => fetchWithPayment(req, u, h), consensusIdenticalAggregation())(providerUrl, paymentTxHash).result();
  if (secondFetch.status !== 200) {
    logState(runtime2, "payment", { error: "provider_rejected", status: secondFetch.status });
    return { ok: false, reason: "Provider rejected payment" };
  }
  let fulfillment;
  try {
    fulfillment = JSON.parse(secondFetch.body);
  } catch {
    return { ok: false, reason: "Invalid fulfillment response" };
  }
  logState(runtime2, "payment", { txHash: paymentTxHash, fulfillment });
  const verified = verifyFulfillment(config, fulfillment);
  if (!verified) {
    logState(runtime2, "verified", { ok: false, reason: "verification_failed" });
    return { ok: false, reason: "Verification failed" };
  }
  logState(runtime2, "verified", { fulfillment });
  ...
  if (config.registryAddress && config.registryAddress.length > 10) {
    try {
      ...
      let hash = 0;
      for (let i2 = 0;i2 < digestBytes.length; i2++) {
        hash = (hash << 5) - hash + digestBytes[i2];
        hash |= 0;
      }
      const digestHex = `0x${Math.abs(hash).toString(16).padStart(64, "0")}`;
      const reportData = encodeAbiParameters(parseAbiParameters("string,string,uint256,string,string,bytes32"), [
        procurementId,
        fulfillment.product,
      ...
      runtime2.log(`Registry tx: ${txHash}`);
    } catch (e) {
      runtime2.log(`Registry write skipped: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  return {
    ok: true,
